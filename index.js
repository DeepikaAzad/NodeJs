...
tripSocket.on('connection', function (socket) {
    ...
    // get lat lng from driver app
    socket.on('driverLocationChange',function(data){
	
	var data = JSON.parse(data);
	console.log(data)
        // send(emit) data to tracking page
        socket.to(data.tripCode).emit('driverLocationChange', data);
    });
    //set room for trip.
